export function parsePrice(text) {
  if (!text) return null;
  // Find first price-shaped token: must have at least one thousands-separator
  // (e.g. 23.800.000 or 23,800,000). Avoids stripping all digits and concatenating.
  const match = String(text).match(/\$?\s*(\d{1,3}(?:[.,]\d{3}){1,3})(?!\d)/);
...
export function parseKm(text) {
  if (!text) return null;
  // "41 mil km" / "1 mil km" — Spanish for thousands (common on Facebook Marketplace)
  const milMatch = String(text).match(/(\d{1,3})\s*mil\s*(?:km|kms)\b/i);
  if (milMatch) {
    const n = parseInt(milMatch[1], 10) * 1000;
    return n >= 0 && n <= 500_000 ? n : null;
  }
  // Match grouped formats (12.345 / 123.456) or bare 1-6 digit runs immediately followed by km
...
export function isRav4Hybrid(title, description = '') {
  ...
  // Reject parts / scrap / wheels — common on Facebook Marketplace
  const isPartsOrScrap = /(desarme|repuesto|llantas?|piezas?|partes?|por parte|chocad|siniestrad)/.test(t);
  if (isPartsOrScrap) return false;
